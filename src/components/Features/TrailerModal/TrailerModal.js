...
export default function TrailerModal(props) {
  ...
  const getTrailer = () => {
    ...
    movie.show_id
      ? axios
          .get(
            `https://api.themoviedb.org/3/tv/${movie.show_id}/season/${movie.season_number}/episode/${movie.episode_number}/videos?api_key=12aa3499b6032630961640574aa332a9&language=en-US`
          )
          .then((results) => {
            console.log(results);
            if (results.data.results[0]) {
              setLoading(false);
              setTrailer(
                `https://www.youtube.com/embed/${results.data.results[0].key}?autoplay=1`
              );
            }
          })
          .catch((error) => {
      ...
      : axios
          ...
          .then((results) => {
            ...
            if (results.data.results[0]) {
              ...
              setLoading(false);
              defaultTrailer.length > 0 &&
                setTrailer(
                  `https://www.youtube.com/embed/${defaultTrailer[0].key}?autoplay=1`
                );
            } else {
              setTrailer(null);
            }
  ...
  const body = trailer ? (
    <iframe src={trailer} allowFullScreen title={props.name}></iframe>
  ) : (
