...
export default function OverviewSection(props) {
    console.log(props);
    let {title, overview, companies, poster_path, vote_average, release_date, homepage, genres, id,  type} = props;
    let homepageOption =
    ...
    let companiesjsx = companies ? companies.map( company =>{
        return <div className="company">{company.logo_path ? <img  src={`https://image.tmdb.org/t/p/original/${company.logo_path}`}></img> : <h4>{company.name}</h4>}</div> 
        }): '';
